...
from smtplib import SMTPResponseException  # Importa la excepción SMTPResponseException
...
def sendMail(html, asunto, para):
    ...
    try:
        smtp_server = smtplib.SMTP(settings.SERVER_STMP, settings.PUERTO_SMTP)
        smtp_server.starttls()
        smtp_server.login(settings.MAIL_SALIDA, settings.PASSWORD_MAIL_SALIDA)
        smtp_server.sendmail(settings.MAIL_SALIDA, para, msg.as_string())
        smtp_server.quit()
